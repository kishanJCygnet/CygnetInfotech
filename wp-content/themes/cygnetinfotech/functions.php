...
$parenthandle = 'twentytwentyone-style'; // This is 'twentyfifteen-style' for the Twenty Fifteen theme.
    ...
    wp_enqueue_style( 'cygnetinfotech-style', get_stylesheet_uri());
}
...
/**
  Function name : cygnetinfotech_setup
  Sets up theme defaults and registers support for various WordPress features.
  Note that this function is hooked into the after_setup_theme hook, which
  runs before the init hook. The init hook is too late for some features, such
  as indicating support for post thumbnail .
 */
function cygnetinfotech_setup()
{
    //Load cygnetinfotech-deparments theme
    load_theme_textdomain('cygnetinfotech');
    //Enable support for Post Thumbnails on posts and pages.
    add_theme_support('post-thumbnails');
    //Add the size of featured image on posts and pages.
    //add_image_size('cygnetinfotech_setup-featured-image', 2000, 1200, true);
    ...
    register_nav_menus(
        array(
            'header_menu' => __('Header Menu', 'cygnetinfotech'),
            'footer_menu' => __('Footer Menu', 'cygnetinfotech'),
            'main_menu' => __('Main Menu', 'cygnetinfotech')
        )
    );
...
add_action('after_setup_theme', 'cygnetinfotech_setup');
